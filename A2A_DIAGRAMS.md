...
>    ✅ **2026-09-01 FinAllQ가 토큰을 동기화해 해소**(44자 일치 확인). ⚠️ 단 **실호출 검증은
>    아직이다** — 스택을 내린 뒤 고쳐서 InsuQ 왕복을 태우지 못했다. 다음 촬영·E2E 때 확인한다.
>
> **스킬별 성공/실패 패턴으로 홉을 좁히는 것**이 이번 진단의 핵심이었다 — 같은 어댑터인데
> 2차 홉이 있는 스킬만 실패하면 원인은 어댑터가 아니라 그 홉에 있다. 다음에도 쓸 수 있다.
>
> ⚠️ **FinAllQ는 InsuQ 인증 실패(401)와 InsuQ 연결 불가(네트워크)를 같은 502로 뭉개 내보낸다.**
> 그래서 화면에는 "FinAllQ A2A 어댑터에 연결할 수 없습니다"로만 뜨고, FinAllQ 자체 장애로
> 오진하게 된다. FinAllQ Pool에 등재됨(2026-09-01).
