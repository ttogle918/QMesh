...
> **🎬 실측 캡처 (2026-08-25 갱신)** — 방어선 ①·②가 실제 화면에서 어떻게 갈리는지 두 세션으로
> 캡처. 위: 설계사(planner) 로그인 → 담당 고객 목록에서 고객 선택 → 고객 상세 페이지의
> "이 고객에게 질문하기"로 "허리 아파서 도수치료 받는데 실비 되나요 한도 있나요" 질의 →
> 근거 조항 인용(파트명·조·항·페이지 전부 포함) + "지급 사유에 해당할 가능성 높음" 3단계
> 판정, "전체 펼치기"로 근거 카드 7개를 한 번에 펼쳐 원문 조항 전체와 하이라이트된 핵심
> 문구까지 확인하는 과정 포함. 아래: "손해위로금도
...
> **🎬 실측 캡처 (2026-08-25) — 실손+화재 동시 가입 고객의 도메인 병합 라우팅.** 어느
> 도메인(실손/화재)인지 애매한 질문은 "라우팅을 포기하고 벡터DB 전체를 무필터로 검색"하는
> 게 아니라, **그 고객이 실제 보유한 도메인 각각을 정확히 스코프해서 따로 검색한 뒤 병합**한다
> (`api/main.py::_qa_candidate_domains` → `classify_domain_llm` → ambiguous 시 실손·화재
> 양쪽 `route_and_answer` → `_merge_ambiguous_results`). `(주)대한물류`(대표자 개인 실손
> `SL-2018-0006` + 화재 `SB-2023-0001` 동시 보유, S28 검증용 시드)에게 "사고가 나서
> 병원비랑 창고 수리비가 같이 나왔는데 둘 다 보상되나요"를 질의 → 근거 36건이 실손 4건 +
> 화재 32건으로 태그돼 병합되고 답변이 `[실손] .../[화재] ...` 형식으로 나오는 것,
> 그리고 인용 항 번호가 근거와 어긋나 방어선②가 "판단 유보"로 강등시키는 것까지 —
> 도메인 병합과 인용 가드레일이 동시에 실전에서 맞물려 도는 순간을 그대로 캡처.
>
> ![도메인 병합 라우팅 실 캡처](assets/insuq-qa-ambiguous-domain-merge.gif)
